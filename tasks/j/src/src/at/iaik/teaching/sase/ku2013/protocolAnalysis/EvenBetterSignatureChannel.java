...
public final class EvenBetterSignatureChannel implements CommunicationChannel {
  ...
  public byte[] transmit(ResourceCheckServer server, byte[] request)
    ...
    // almost the same replay attack as in BetterSignatureChannel is possible in this environment,
    // due to the fact, that the nonce is not part of the content that gets signed by the server.
    ...
    String nonce = resp.split("\n")[3].replaceAll("\\s+","").split("=")[1];
